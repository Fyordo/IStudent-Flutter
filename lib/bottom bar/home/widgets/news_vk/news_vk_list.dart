...
class NewsVkList extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return BlocProvider<NewsVkBloc>(
      create: (context) {
        return NewsVkBloc()..add(NewsVkEventWithNews());
      },
      child: BlocListener<RefresherBloc, RefresherState>(
        listener: (context, state) {
          BlocProvider.of<NewsVkBloc>(context).add(NewsVkEventWithNews());
        },
        child: BlocBuilder<NewsVkBloc, NewsVkState>(
            ...
            if (state is NewsVkStateWithNews) {
              return Container(
                  ...
                  child: ListView.separated(
                    ...
                    itemBuilder: (BuildContext context, int index) =>
                        GestureDetector(
                      onTap: () {
                        IStudent.launchURL(state.news_vk[index].url);
                      },
                      child: ListTile(
                        title: Text(state.news_vk[index].text != ""
                            ? state.news_vk[index].text.substring(0,
                                    min(55, state.news_vk[index].text.length)) +
                                "..."
              ...
              return Column(
                children: [
                  ...
                  Text("Загрузка виджета новостей СИЦа..."),
                  Container(
                    height: 20,
                  ),
